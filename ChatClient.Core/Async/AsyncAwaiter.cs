...
{
	/// <summary>
	/// Adds the ability to safely await on tasks to be complite that need limited access
	/// For example, only allowing one task to access some data at a time, like the old
	/// asynchronous locks
	/// 
	/// This awaiter uses the safer samaphore to prevent any chance of deadlock
	/// </summary>
	public static class AsyncAwaiter
	{
		...
		/// <summary>
		/// Awaits for any outstanding tasks to comlete that are accessing the same key then runs the given task, returning it's value
		/// </summary>
		/// <param name="key">The key to awiat</param>
		/// <param name="task">The task to perform insidre of the semaphore lock</param>
		/// <param name="maxAccessCount">If this is the first call, sets the maximum numberof tasks that access this task before is waiting</param>
		/// <returns></returns>
		public static async Task<T> AwaitResultAsync<T>(string key, Func<Task<T>> task, int maxAccessCount = 1)
		{
			...
			finally
			{
				...
				// This is why it is important to do the Release within a try...finaly clause
			...
			// Now use this semaphore and preform the desired task inside its lock
		...
		/// <summary>
		/// Awaits for any outstanding tasks to comlete that are acccessing the same key runs the given task
		/// </summary>
		/// <param name="key">The key to await</param>
		/// <param name="task">The task to perform inside of the semaphore lock</param>
		/// <param name="maxAccessCount">If this is the first call, sets the maximum number of tasks that can access this task before is awaiting</param>
		/// <returns></returns>
